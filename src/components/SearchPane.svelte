<div class="search-pane">
    <input on:keyup={searchAction} bind:this={searchBox} class="search body-large" type="text" placeholder="Search Device">  
    {#each filteredList as device}
    <a href={`/downloads/${device}`}>
        <div class="button">
            <div class="btn body-large" class:active={activeDevice === device}>{device}</div>
        </div>
    </a>
    {/each}
</div>
